...
local CSV_HEADERS =
    "Coords,Lat,Long,Seed,xxhashShuffleBreakThroughTech,SiteName,Locales,TerrainType,Topography,Rating,Altitude,Temperature,Difficulty," ..
    "MetalsBars,ConcreteBars,WaterBars,DustDevilsBars,DustStormBars,MeteorBars,ColdWaveBars," ..
    "Metals,Concrete,Water,DustDevils,DustStorm,Meteor,ColdWave," ..
    "MapTemplateID"
...
local function GetTopographyAndRating(map_template_id)
    if not map_template_id or map_template_id == "Unknown" then
        return "Unknown", 0
    end
...
local function FormatCoordinates(lat, long)
    ...
    -- Matches the game's coordinate notation
    local lat_dir = lat_deg >= 0 and "S" or "N"
    local long_dir = long_deg >= 0 and "E" or "W"

    local coord_str = string.format("%d%s%d%s",
        math.abs(lat_deg),
        lat_dir,
        math.abs(long_deg),
        long_dir
            ...
            CreateRealTimeThread(function()
                ...
                for lat = -70 * 60, 70 * 60, 60 do
                    for long = -180 * 60, 180 * 60, 60 do
                        ...
                        if p then
                            local coord_str, lat_deg, long_deg =
                                FormatCoordinates(lat, long)

                            local site_name =
                                GetSiteName(lat, long, p.Locales)

                            local gen =
                                (type(GetRandomMapGenerator) == "function"
                                    and GetRandomMapGenerator())
                                or { Seed = p.Seed or 0 }

                            local seed_val = p.Seed or 0

                            -- Raw 64-bit hash used for breakthrough shuffle
                            local xxhashShuffleBreakThroughTech =
                                xxhash(seed_val, "ShuffleBreakThroughTech")

                            local map_template_id = "Unknown"
                            pcall(function()
                                map_template_id =
                                    FillRandomMapProps(gen, p)
                            end)

                            local topography, rating =
                                GetTopographyAndRating(map_template_id)
                            ...
                            local row_data = {
                                ...
                                long_deg,
                                seed_val,
                                tostring(xxhashShuffleBreakThroughTech),
                                site_name,
                                p.Locales or 0,
                                p.TerrainType or "Unknown",
                                topography,
                                rating,
                                p.Altitude or 0,
                                p.Temperature or 0,
                                (g_TitleObj and g_TitleObj:GetDifficultyBonus()) or 0,

                                -- Bars
                                GetBars(p.Metals),
                                GetBars(p.Concrete),
                                GetBars(p.Water),
                                GetBars(p.DustDevils),
                                GetBars(p.DustStorm),
                                GetBars(p.Meteor),
                                GetBars(p.ColdWave),
                ...
                if landing and landing.delete then
                    landing:delete()
                end
